...
PolicyAccount.post("/add-policy-account", async (req: Request, res: any) => {
  try {
    delete req.body.mode;
    delete req.body.search;
    req.body.createdAt = new Date();
    const policyAccounts = (await checkedAccountIsExisting(
      req.body.Account
    )) as Array<any>;
...
PolicyAccount.post("/update-policy-account", async (req: Request, res: any) => {
  try {
    if (
...
PolicyAccount.post("/delete-policy-account", async (req: Request, res: any) => {
  try {
    if (
      !(await saveUserLogsCode(
        req,
